...
class KuaiShouApp(AppRunCommon):
    ...
    def __init__(self, device: DeviceManager):
        self.device = device
        super().__init__(AppInfoKuaiShou, device)
    ...
    def main_task_human(self, star: bool, comment: bool, works: bool):
        thumbs_up_id = "com.kuaishou.nebula:id/like_icon"
        comment_id = "com.kuaishou.nebula:id/comment_icon"
        ...
        works_list_id = "com.kuaishou.nebula:id/recycler_view"
        if star:
            self.device.click_by_flag(thumbs_up_id)
        if comment:
            self.device.click_by_flag(comment_id)
        if works:
            if self.device.ui_operation_sequence(go_works_list, works_list_success):
                item = self.device.find_list_by_child(works_list_id)
                if item:
                    item.click()
